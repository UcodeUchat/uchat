...
static int mx_exit_stream(t_audio *data, PaError err) {
...
static int mx_init_stream(PaStream **stream, t_audio *data, t_a_snippet *sample_block) {
    PaError err;
...
static long mx_save_audio(t_audio *data) {
    ...
    char file_name[100];
    snprintf(file_name, 100, "%d.aif", rand());  //rand -> replace by message id
    printf("start save audio\n");
    SNDFILE *outfile = sf_open(file_name, SFM_WRITE, &sfinfo);
    if (!outfile) {
        printf("error outfile =%d\n", sf_error(outfile));
...
static float mx_change_threshold(float talking_threshold, float talking_ntensity) {
    return TALKING_THRESHOLD_WEIGHT * talking_threshold +
           (1 - TALKING_THRESHOLD_WEIGHT) * talking_ntensity /
           TALKING_TRIGGER_RATIO;
}

static float mx_rms(float *data, size_t len) {
...
/*
// now not useds
static int mx_process_stream_ext(PaStream *stream, t_audio *data,

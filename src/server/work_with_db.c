...
int mx_sign_in(t_server_info *i, json_object *js, int sock) {
    ...
    if (sqlite3_exec(i->db, command, check_data, js, 0) != SQLITE_OK)
        return -1;
    if ((mx_update_socket(i, sock, json_object_get_string(json_object_object_get(js, "login")))) == -1)
...
int mx_check_client(t_server_info *info, json_object *js, int sock) {
    if ((mx_find_sock_in_db(info, json_object_get_string(json_object_object_get(js, "login")))) == 1) {

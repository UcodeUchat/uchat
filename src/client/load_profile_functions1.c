...
void mx_show_login (t_client_info *info, json_object *new_json, GtkWidget *profile) {
    ...
    gtk_box_pack_start (GTK_BOX (login_box), info->data->profile->login_entry, 
                        FALSE, FALSE, 0);
    gtk_widget_show(info->data->profile->login_entry);
    gtk_editable_set_editable (GTK_EDITABLE (info->data->profile->login_entry), 
                                FALSE);
...
static void show_email_settings(t_client_info *info, GtkWidget *email_box) {
    ...
    GdkPixbuf *pixbuf = gdk_pixbuf_new_from_file_at_scale ("img/edit.png", 
                                                        22, 22, TRUE, NULL);
    GtkWidget *image = gtk_image_new_from_pixbuf(pixbuf);
    gtk_button_set_image(GTK_BUTTON(info->data->profile->email_button1), image);
    gtk_box_pack_start (GTK_BOX (email_box), info->data->profile->email_button1, 
                        FALSE, FALSE, 0);
    ...
    gtk_box_pack_start (GTK_BOX (email_box), info->data->profile->email_button2, 
                        FALSE, FALSE, 0);
...
void mx_show_email (t_client_info *info, int id, json_object *new_json, 
                    GtkWidget *profile) {
...
void mx_show_id (t_client_info *info, int id, GtkWidget *profile) {
    ...
    gtk_box_pack_start (GTK_BOX (id_box), info->data->profile->id_entry, 
                        FALSE, FALSE, 0);
    gtk_widget_show(info->data->profile->id_entry);
    gtk_editable_set_editable (GTK_EDITABLE (info->data->profile->id_entry), 
                                FALSE);

...
static void show_name_settings(t_client_info *info, GtkWidget *name_box) {
    ...
    GdkPixbuf *pixbuf = gdk_pixbuf_new_from_file_at_scale ("img/edit.png", 22, 
                                                            22, TRUE, NULL);
    GtkWidget *image = gtk_image_new_from_pixbuf(pixbuf);
    gtk_button_set_image(GTK_BUTTON(info->data->profile->name_button1), image);
    gtk_box_pack_start (GTK_BOX (name_box), info->data->profile->name_button1, 
                        FALSE, FALSE, 0);
    gtk_widget_show(info->data->profile->name_button1);

    info->data->profile->name_button2 = gtk_button_new();
    pixbuf = gdk_pixbuf_new_from_file_at_scale ("img/save.png", 
                                                22, 22, TRUE, NULL);
    image = gtk_image_new_from_pixbuf(pixbuf);
    gtk_button_set_image(GTK_BUTTON(info->data->profile->name_button2), image);
    gtk_box_pack_start (GTK_BOX (name_box), info->data->profile->name_button2, 
                        FALSE, FALSE, 0);
...
void mx_show_name (t_client_info *info, int id, json_object *new_json, 
                    GtkWidget *profile) {
    ...
    gtk_box_pack_start (GTK_BOX (name_box), info->data->profile->name_entry, 
                        FALSE, FALSE, 0);

...
static void init_create_entry (t_client_info *info, GtkWidget *box) {
    ...
    gtk_entry_set_placeholder_text (GTK_ENTRY (info->data->create_room->name_entry), 
                                    "Write room name");
    ...
    gtk_box_pack_start (GTK_BOX (box1), info->data->create_room->name_entry, 
                        TRUE, FALSE, 0);
...
static void init_create_combo_box (t_client_info *info, GtkWidget *box) {
    ...
    gtk_combo_box_text_append (GTK_COMBO_BOX_TEXT
                                (info->data->create_room->selection), "0", "Public");
    gtk_combo_box_text_append (GTK_COMBO_BOX_TEXT
                                (info->data->create_room->selection), "1", "Private");
    gtk_combo_box_set_active (GTK_COMBO_BOX(info->data->create_room->selection), 0);
    gtk_box_pack_start (GTK_BOX (box1), info->data->create_room->selection, 
                        TRUE, FALSE, 0);
...
static void init_create_buttons (t_client_info *info, GtkWidget *box) {
    ...
    g_signal_connect(G_OBJECT(info->data->create_room->create_button), 
                    "clicked", G_CALLBACK(mx_create_room_callback), info);
    ...
    g_signal_connect(G_OBJECT(info->data->create_room->cancel_button), 
                    "clicked", G_CALLBACK(mx_close_creation_callback1), info);
    gtk_box_pack_start (GTK_BOX (box1), 
                        info->data->create_room->cancel_button, TRUE, FALSE, 0);
...
void mx_init_create (t_client_info *info) { 
    ...
    gtk_fixed_put(GTK_FIXED(info->data->general_box), 
                    info->data->create_room->main_box, 0, 0);
    ...
    main_box = mx_init_menu_main_box(info, info->data->create_room->main_box, 
                                    "profile");

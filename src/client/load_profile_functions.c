...
static void init_check_boxes (t_client_info *info, json_object *new_json, 
    ...
    gtk_box_pack_start (GTK_BOX (notifications_box), info->data->profile->visual, 
                        FALSE, FALSE, 0);
    ...
    gtk_box_pack_start (GTK_BOX (notifications_box), info->data->profile->audio, 
                        FALSE, FALSE, 0);
    ...
    gtk_box_pack_start (GTK_BOX (notifications_box), info->data->profile->email, 
                        FALSE, FALSE, 0);
    info->data->profile->save = gtk_button_new_with_label("Save");
    g_signal_connect (G_OBJECT (info->data->profile->save), "clicked", 
                        G_CALLBACK (save_settings_callback), info);
    gtk_box_pack_start (GTK_BOX (notifications_box), info->data->profile->save, 
                        FALSE, FALSE, 0);
}
...
void mx_show_global_settings (t_client_info *info, json_object *new_json, GtkWidget *profile) {
    ...
    gtk_orientable_set_orientation (GTK_ORIENTABLE(notifications_box), 
                                    GTK_ORIENTATION_VERTICAL);
...
void mx_show_etc (t_client_info *info, json_object *new_json, GtkWidget *profile) {
    ...
    gtk_orientable_set_orientation (GTK_ORIENTABLE(box), 
                                    GTK_ORIENTATION_VERTICAL);
    gtk_orientable_set_orientation (GTK_ORIENTABLE(activityes_box), 
                                    GTK_ORIENTATION_VERTICAL);
    ...
    g_signal_connect (G_OBJECT (button), "clicked", 
                        G_CALLBACK (message_callback), info);

...
static void init_edit_button (t_client_info *info, 
                                char *i_name, GtkWidget *fixed_message) {
    ...
    g_signal_connect(G_OBJECT(info->data->edit_button), "button_press_event", 
                        G_CALLBACK(mx_edit_cancel_callback), info);
...
void mx_init_message_box (t_client_info *info, GtkWidget *box, 
    ...
    gtk_entry_set_placeholder_text(GTK_ENTRY (info->data->message_entry), 
                                    "Write something");
    ...
    g_signal_connect(G_OBJECT(info->data->message_entry),"activate", 
...
void mx_init_general_button (t_client_info *info, char *i_name, GtkWidget *box,
    ...
    GdkPixbuf *pixbuf = gdk_pixbuf_new_from_file_at_scale (i_name, 20, 20, 
                                                            TRUE, NULL);
    GtkWidget *image = gtk_image_new_from_pixbuf(pixbuf);
...
void mx_init_notebook (t_client_info *info, GtkWidget *box) {
    ...
    gtk_widget_set_size_request(info->data->notebook, 
                                gtk_widget_get_allocated_width 
                                (info->data->window) -  20, 550);

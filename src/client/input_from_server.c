...
int show_message(GtkWidget *widget) {
    gtk_widget_show(widget);
    return 0;
}

int destroy_message(GtkWidget *widget) {
...
void delete_message(t_client_info *info, json_object *new_json) { 
    ...
    if (message_id >= room->messages->id) {
        ...
        g_idle_add ((GSourceFunc)destroy_message, message->h_box);
...
void show_login_settings(t_client_info *info, GtkWidget *login_box) {
...
void show_login (t_client_info *info, int id, json_object *new_json, GtkWidget *profile) {
    const char *login = json_object_get_string(json_object_object_get(new_json, "login"));
    ...
    gtk_editable_set_editable (GTK_EDITABLE (info->data->profile->login_entry), FALSE);
    if (id == info->id)
        show_login_settings(info, login_box);
...
void load_user_profile(t_client_info *info, json_object *new_json) {
    ...
    show_login(info, id, new_json, profile);

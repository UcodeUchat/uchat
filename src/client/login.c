...
GtkWidget *init_delete_menu (t_client_info *info) {
...
void init_delete_button (t_client_info *info, GtkWidget *box, char *text,
    ...
    gtk_widget_set_halign(box1, GTK_ALIGN_CENTER);
    gtk_box_pack_start(GTK_BOX (box), box1, TRUE, FALSE, 0);
    g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(callback), answer_menu);
    gtk_box_pack_start(GTK_BOX (box1), button, TRUE, FALSE, 0);
    gtk_widget_set_size_request(button, 100, -1);
    gtk_widget_set_name(button, "entry");
    gtk_widget_show(button);
...
void init_menu (t_client_info *info) {
    ...
    init_delete_button (info, box, "Delete acc", delete_acc_callback);

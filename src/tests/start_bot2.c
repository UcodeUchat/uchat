...
int mx_tls_config_client(t_client_info *info) {
    ...
    if ((config = tls_config_new()) == NULL)
        return mx_err_return("unable to allocate config");
    if ((info->tls_client = tls_client()) == NULL)
        return mx_err_return("tls client creation failed");
    tls_config_insecure_noverifycert(config);
    tls_config_insecure_noverifyname(config);
    if (tls_config_set_dheparams(config, "auto") != 0)
        return mx_err_return(tls_config_error(config));
    ...
    if (tls_configure(info->tls_client, config) != 0)
        return mx_err_return2("tls_configure error: ", tls_error(info->tls_client));
...
int mx_make_tls_connect_client(t_client_info *info) {
    if (tls_connect_socket(info->tls_client, info->socket, "uchat") < 0)
        return mx_err_return2("tls_connect error: ", tls_error(info->tls_client));
    if (tls_handshake(info->tls_client) < 0)
        return mx_err_return2("tls_handshake error: ", tls_error(info->tls_client));
    return 0;
}

...
void mx_init_general (t_client_info *info) {
    ...
    gtk_fixed_put(GTK_FIXED(info->data->main_box), 
                    info->data->general_box, 0, 0);
    ...
    mx_init_general_button (info ,"img/search.png", box, 
                            mx_show_search_callback);

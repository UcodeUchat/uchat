...
void mx_image (t_message *node) {
    ...
    gtk_box_pack_start (GTK_BOX (node->message_box), node->image_box, 
                        FALSE, FALSE, 0);
}

void mx_sticker (t_message *node, const char *message) {
    GtkWidget *stick_box =  gtk_box_new(FALSE, 0);
    GdkPixbuf *item_pixbuf = gdk_pixbuf_new_from_file_at_scale (message, 
                            200, 200, TRUE, NULL);
    GtkWidget *item_image = gtk_image_new_from_pixbuf(item_pixbuf);

    gtk_widget_show(stick_box);
    gtk_box_pack_start (GTK_BOX (node->central_box), stick_box, 
                        FALSE, FALSE, 0);
